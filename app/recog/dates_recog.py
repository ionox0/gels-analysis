...
def sort_rects(rects_ctrs):
    rects_sort = sorted(rects_ctrs, key=lambda r: r[0][0] + r[0][2])

    result = []
    for rect in rects_sort:
        # Filter to only rects with overlap of > 1/3 of target height
        rects_sort_filt = filter(
            lambda x: calc_overlap(x[0], rect[0]) > (x[0][3] / 3.0) and abs(x[0][3] - rect[0][3]) < (x[0][3]),
            rects_sort)
...
def extract_numbers(fname, thresh=50, blur=True):
    result = []
    ...
    ctrs = cv2.findContours(im_th_all_colors, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
